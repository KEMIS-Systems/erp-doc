---
title: Gestão de estoque e patrimônio
slug: /packs/inventory-and-asset-management
sidebar_position: 5
---

# Gestão de estoque e patrimônio

Plano para controle de almoxarifado, patrimônio e EPIs, indicado para empresas que precisam rastrear suprimentos, ativos e equipamentos de proteção individual entregues aos colaboradores.

## Páginas incluídas

### Administrativo

* [Grupos de acesso](../administrative/access-groups.md)
* [Usuários](../administrative/users.md)
* [Departamentos](../administrative/departments.md)

### Almoxarifado

* **Grupos** _(manual deste módulo ainda não publicado)_
* **Unidades de medida** _(manual deste módulo ainda não publicado)_
* **Suprimentos** _(manual deste módulo ainda não publicado)_
* **Tipos de inventários** _(manual deste módulo ainda não publicado)_
* **Inventários** _(manual deste módulo ainda não publicado)_
* **Estoque** _(manual deste módulo ainda não publicado)_

### Patrimônio

* **Grupos** _(manual deste módulo ainda não publicado)_
* **Modelos** _(manual deste módulo ainda não publicado)_
* **Patrimônio** _(manual deste módulo ainda não publicado)_

### EPI

* **Grupos** _(manual deste módulo ainda não publicado)_
* **EPI** _(manual deste módulo ainda não publicado)_
* **Tipos de inventários** _(manual deste módulo ainda não publicado)_
* **Inventários** _(manual deste módulo ainda não publicado)_
* **Solicitações de EPI** _(manual deste módulo ainda não publicado)_
